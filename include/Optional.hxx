...
template<class T, std::enable_if_t<!has_overloaded_address_operator_v<T>>* = nullptr>
...
namespace
{
	...
	template<class T>
	union basic_constexpr_storage_layout<T, void_t<std::enable_if_t<std::is_trivially_destructible<T>::value>>>
	{
		private:
		uint8_t dummy_;
...
class optional : basic_storage<T>
{
	...
	public:
	constexpr optional() noexcept(std::is_nothrow_default_constructible<T>::value) = default;
